...
def answer(minions):
    # Formula: (time to question) / (probability they answer)
    indeces = range(len(minions))
    calculated_minions = list(indeces)
    for i,x in enumerate(minions):
        calculated_minions[i] = float(minions[i][0])*float(minions[i][2])/float(minions[i][1])
        
    return [indeces for (calculated_minions,indeces) in sorted(zip(calculated_minions,indeces))]